<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Dự án mẫu</title>
    <link rel="stylesheet" href="../css/css.css">
    <link href="https://fonts.googleapis.com/css2?family=Roboto:wght@300;400;500;700;900&display=swap" rel="stylesheet">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-T3c6CoIi6uLrA9TneNEoa7RxnatzjcDSCmG1MXxSR1GAsXEV/Dwwykc2MPK8M2HN" crossorigin="anonymous">
    <script src="https://www.gstatic.com/charts/loader.js"></script>
</head>

<body>
    <div class="boxcenter">
        <!-- BIGIN HEADER -->
        <header style="background: none !important;">
            <div class="row mb header_admin">
                <h1>Admin</h1>
            </div>
            <a style="margin-bottom: 30px; margin-left: 0 !important;" href="/duanmau/duanmau/index.php" class="btn btn-success">Quay về trang người dùng</a>
            <div class="row mb menu">
                <ul>
                    <li><a href="index.php">Trang chủ</a></li>
                    <li><a href="index.php?act=listdanhmuc">Danh mục</a></li>
                    <li><a href="index.php?act=listsanpham">Hàng hóa</a></li>
                    <li><a href="index.php?act=listkhachhang">Khách hàng</a></li>
                    <li><a href="index.php?act=danhsachbinhluan">Bình luận</a></li>
                    <li><a href="index.php?act=thongke">Thống kê</a></li>
                </ul>
            </div>
        </header>
        <!-- END HEADER -->